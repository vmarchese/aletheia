...
class LLMClient:
    ...
    def __init__(self, agent_name: str = None):
        ...
        if "bedrock" in endpoint and BEDROCK_AVAILABLE:
            ...
            self._client = BedrockChatClient(
                model_id=self.model, 
                region=region
            )
    ...
    def get_client(self):
        ...
        if self._client is None:
            if not BEDROCK_AVAILABLE and "bedrock" in os.environ.get("ALETHEIA_OPENAI_ENDPOINT", ""):
                raise ValueError("Bedrock support requires agent-framework-bedrock. Install with: uv pip install agent-framework-bedrock --pre")
