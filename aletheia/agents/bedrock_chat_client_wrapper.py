...
class ChatOptions:
    ...
    @property
    def allow_multiple_tool_calls(self) -> bool | None:
    ...
    @property
    def additional_properties(self) -> dict | None:
...
class BedrockChatClientTraceWrapper:
    ...
    def __init__(self, chat_client):
        ...
        self._original_get_streaming_response = (
            chat_client._inner_get_streaming_response
        )
                    ...
                    if data is not None:
                        try:
                            if hasattr(data, "__dict__"):
                                ...
                                f.write(
                                    f"BEDROCK_TRACE_DATA: {json.dumps(data.__dict__, indent=2, default=str)}\n"
                                )
                            else:
                                # For other data types
                                f.write(
                                    f"BEDROCK_TRACE_DATA: {json.dumps(data, indent=2, default=str)}\n"
                                )
                        except Exception as e:
                            f.write(
                                f"BEDROCK_TRACE_DATA: {str(data)} (JSON serialization failed: {e})\n"
                            )
                    f.write("---\n")
        except Exception as e:
    ...
    def _dump_input_messages(
        ...
        try:
            ...
            for i, msg in enumerate(messages):
                msg_data = {"index": i, "role": str(msg.role), "contents": []}

                if hasattr(msg, "contents") and msg.contents:
                    for j, content in enumerate(msg.contents):
                        content_info = {"index": j, "type": type(content).__name__}
    ...
    def _clean_messages_for_bedrock(
        ...
        for i, msg in enumerate(messages):
            ...
            if msg.role == "tool" or str(msg.role).lower() == "tool":
                ...
                for content in msg.contents:
                    call_id = getattr(
                        content, "call_id", getattr(content, "tool_use_id", None)
                    )
                    if not call_id:
                        continue
                ...
                if internal_calls:
                    ...
                    self._log_trace(
                        f"Found {len(internal_calls)} internal tool calls in message {i}: {internal_calls}"
                    )
        ...
        for i, msg in enumerate(messages):
            ...
            if is_tool_message:
                ...
                if tool_calls and tool_results:
                    self._log_trace(
                        f"Message {i} is a MIXED-CONTENT tool message with {len(tool_calls)} tool calls and {len(tool_results)} tool results - CONVERTING tool calls to text"
                    )
                    ...
                    self._log_trace(
                        f"Found {len(paired_results)} paired results and {len(orphaned_results)} orphaned results"
                    )
                    ...
                    for tool_call in tool_calls:
                        call_id = getattr(
                            tool_call,
                            "call_id",
                            getattr(tool_call, "tool_use_id", "unknown"),
                        )
                    ...
                    for tool_result in paired_results:
                        call_id = getattr(
                            tool_result,
                            "call_id",
                            getattr(tool_result, "tool_use_id", "unknown"),
                        )
                    ...
                    if user_contents:
                        ...
                        self._log_trace(
                            f"Created user message with {len(orphaned_results)} orphaned tool results, {len(text_contents)} text contents (including {len(tool_calls)} converted tool calls and {len(paired_results)} converted paired results)"
                        )
            ...
            for content in msg.contents:
                ...
                if effective_role == "assistant":
                    ...
                    if is_tool_result:
                        self._log_trace(
                            f"Filtering out tool result from assistant message {i}"
                        )
                        if call_id:
                    ...
                    if is_tool_use:
                        ...
                        if call_id and call_id in filtered_call_ids:
                            self._log_trace(
                                f"Filtering out internal toolUse from {msg.role} message {i} (call_id: {call_id})"
                            )
                        else:
                            self._log_trace(
                                f"Filtering out tool use from {msg.role} message {i} (will be user in Bedrock)"
                            )
                    ...
                    if is_tool_result and call_id and call_id in filtered_call_ids:
                        self._log_trace(
                            f"Filtering out internal toolResult from {msg.role} message {i} (call_id: {call_id})"
                        )
                        continue
            ...
            if new_contents:
                ...
                self._log_trace(
                    f"Message {i} ({msg.role}->{effective_role}): kept {len(new_contents)}/{len(msg.contents)} contents"
                )
            else:
                self._log_trace(
                    f"Message {i} ({msg.role}): filtered out completely (no valid content)"
                )
        ...
        for i, msg in enumerate(cleaned_messages):
            ...
            # If this is a user message, validate all tool results have corresponding tool uses
                ...
                for content in msg.contents:
                    ...
                    if is_tool_result:
                        ...
                        if call_id and call_id in available_tool_uses:
                            ...
                            self._log_trace(
                                f"Message {i}: Tool result {call_id} is valid (has corresponding tool use)"
                            )
                        else:
                            # Orphaned - no corresponding tool use found
                            orphaned_results.append(call_id)
                            self._log_trace(
                                f"Message {i}: Tool result {call_id} is ORPHANED (no corresponding tool use) - FILTERING OUT"
                            )
                    else:
                ...
                if valid_contents:
                    ...
                    if orphaned_results:
                        self._log_trace(
                            f"Message {i}: Filtered out {len(orphaned_results)} orphaned tool results"
                        )
                else:
                    self._log_trace(
                        f"Message {i}: Completely filtered out (only had orphaned tool results)"
                    )
        ...
        self._log_trace(
            f"Validation complete: {len(cleaned_messages)} -> {len(validated_messages)} messages"
        )
    ...
    async def _traced_get_streaming_response(
        ...
        self._log_trace("=== WRAPPER RECEIVED ===")
        self._log_trace(f"options dict keys: {list(options.keys())}")
        self._log_trace(f"options['response_format']: {options.get('response_format')}")
        self._log_trace(f"kwargs keys: {list(kwargs.keys())}")
        self._log_trace(
            f"kwargs.get('response_format'): {kwargs.get('response_format')}"
        )
        ...
        self._log_trace(
            f"Tools available: {len(chat_options.tools) if chat_options.tools else 0}"
        )
        ...
        for i, msg in enumerate(messages):
            if hasattr(msg, "contents") and msg.contents:
                ...
                for content in msg.contents:
                    ...
                    is_func_call = "FunctionCall" in ctype or (
                        hasattr(content, "name") and hasattr(content, "call_id")
                    )
                    is_func_result = "FunctionResult" in ctype or (
                        hasattr(content, "call_id") and not hasattr(content, "name")
                    )
                ...
                self._log_trace(
                    f"Message {i} BEFORE: role={msg.role}, contents=[{', '.join(content_types)}]"
                )
        ...
        if chat_options.max_tokens is None or chat_options.max_tokens < 8192:
            self._log_trace(
                f"Increasing max_tokens from {chat_options.max_tokens} to 8192 to avoid truncation"
            )
        ...
        if kwargs_tool_choice == "none" and self._cached_tools:
            self._log_trace(
                "⚠️  DETECTED: kwargs has tool_choice='none' from previous call"
            )
            self._log_trace(
                "   This will cause empty responses even with tools available"
            )
            self._log_trace(
                "   Removing tool_choice from kwargs to allow fresh evaluation"
            )
        ...
        if chat_options.tools is None or len(chat_options.tools) == 0:
            if self._cached_tools:
                ...
                self._log_trace(
                    f"No tools provided but have cached tools - restoring {len(self._cached_tools)} tools"
                )
                self._log_trace(f"Reason: {reason}")
                self._log_trace(
                    f"Updating tool_choice from '{chat_options.tool_choice}' to '{new_tool_choice}'"
                )

                # Update options with restored tools and new tool_choice
                chat_options.update(
                    tool_choice=new_tool_choice, tools=self._cached_tools
                )
            elif has_tool_content_in_history:
                # No cached tools but history has tool content - this will fail
                self._log_trace(
                    "WARNING: No tools provided and no cached tools available, but conversation has tool content"
                )
                self._log_trace("This will cause Bedrock API error")
            else:
                # No tools, no cached tools, no tool content - safe to proceed without tools
                if chat_options.tool_choice is not None:
                    self._log_trace(
                        "No tools and no tool content in history, setting tool_choice to None"
                    )
        ...
        for i, msg in enumerate(messages):
            ...
            self._log_trace(
                f"Contents count: {len(msg.contents) if hasattr(msg, 'contents') and msg.contents else 0}"
            )
            ...
            if hasattr(msg, "contents") and msg.contents:
                for j, content in enumerate(msg.contents):
                    ...
                    is_function_call = "FunctionCall" in content_type or (
                        hasattr(content, "name") and hasattr(content, "call_id")
                    )
                    is_function_result = "FunctionResult" in content_type or (
                        hasattr(content, "call_id") and not hasattr(content, "name")
                    )

                    if is_function_call:
                        self._log_trace(
                            f"⚠️  FUNCTION CALL DETECTED in {msg.role} message"
                        )
                        if hasattr(content, "name"):
        ...
        if chat_options.tools:
            self._log_trace("--- Available Tools ---")
        ...
        def traced_prepare_options(*args, **kwargs):
            ...
            if tool_config:
                ...
                if isinstance(tool_choice, dict) and "none" in tool_choice:
                    self._log_trace(
                        f"⚠️  FIXING INVALID TOOL_CHOICE: Detected toolChoice={tool_choice}"
                    )
                    self._log_trace("   Bedrock doesn't support 'none'")
                    self._log_trace(
                        "   Converting to 'auto' - let model decide whether to use tools"
                    )
                    ...
                    self._log_trace(
                        "   ✅ toolChoice changed to 'auto' - model can choose"
                    )

            # FIX: Add response_format to Bedrock request if specified
            # The agent-framework-bedrock doesn't currently support response_format,
            # so we need to add it manually to the request
            #
            # IMPORTANT: Only add schema when there are NO tool calls in the conversation
            # If there are tool calls, the model needs to be free to use tools first
            # before generating the final structured response
            if chat_options.response_format:
                # Check if this is a tool execution phase or final response phase
                # If tools are available and tool_choice is "auto", the model might want to use tools
                # We should NOT force JSON schema in that case

                # Detect if tools have been executed by checking for FunctionResultContent
                # in any message (not just role=TOOL messages, since results can be in assistant messages)
                    ...
                    if hasattr(msg, "contents") and msg.contents:
                        for content in msg.contents:
                            ...
                            if is_function_result:
                                ...
                                self._log_trace(
                                    f"   Found tool result in message (role={msg.role})"
                                )
                                break
                    if has_tool_results:
                        break
                ...
                # Only add schema if:
                # 1. There are tool results in the conversation (model has already used tools)
                # 2. OR there are no tools available (pure response mode)
                should_add_schema = has_tool_results or not chat_options.tools

                if should_add_schema:
                    self._log_trace(
                        f"⚠️  ADDING RESPONSE_FORMAT: {chat_options.response_format}"
                    )

                    # Get the schema from the response_format (it's a Pydantic model)
                    try:
                        if hasattr(chat_options.response_format, "model_json_schema"):
                            schema = chat_options.response_format.model_json_schema()
                            self._log_trace("   Schema extracted from Pydantic model")

                            # For Bedrock Converse API, we add schema to system prompt
                            # This guides the model to respond with structured JSON
                            system_prompts = request.get("system", [])
...
"""
                            ...
                            # Add schema instruction to system prompts
                            system_prompts.append({"text": schema_instruction})
                            request["system"] = system_prompts

                            self._log_trace(
                                "   ✅ Schema added to system prompt for guided generation"
                            )
                            self._log_trace(
                                f"   Schema name: {schema.get('title', 'response')}"
                            )
                        else:
                            self._log_trace(
                                "   ⚠️  response_format doesn't have model_json_schema method"
                            )
                    except Exception as e:
                    ...
                    self._log_trace(
                        "⚠️  SKIPPING response_format: Model needs to use tools first"
                    )
                    self._log_trace(
                        f"   has_tool_results={has_tool_results}, has_tools={bool(chat_options.tools)}"
                    )
                    if chat_options.tools:
                        self._log_trace(
                            f"   Available tools: {len(chat_options.tools)}"
                        )
                        # Log first few tool names for debugging
                        for tool in chat_options.tools[:3]:
                            tool_name = (
                                tool.get("name", "unknown")
                                if isinstance(tool, dict)
                                else getattr(tool, "name", "unknown")
                            )
                            self._log_trace(f"     - {tool_name}")
                        if len(chat_options.tools) > 3:
                            self._log_trace(
                                f"     ... and {len(chat_options.tools) - 3} more"
                            )
                ...
                for j, content in enumerate(contents):
                    if isinstance(content, dict):
                        ...
                        if content_type == "toolUse":
                            has_tool_use = True
                            self._log_trace(
                                f"⚠️  TOOL_USE: id={content.get('id')}, name={content.get('name')}"
                            )
                        elif content_type == "toolResult":
                            has_tool_result = True
                            self._log_trace(
                                f"TOOL_RESULT: toolUseId={content.get('toolUseId')}"
                            )
                            ...
                            self._log_trace(
                                f"UNKNOWN TYPE: {content_type}, keys: {list(content.keys())}"
                            )

                # Validate message content based on role
                if role == "user" and has_tool_use:
                    self._log_trace(
                        f"❌ VALIDATION ERROR: User message {i} contains toolUse - THIS WILL FAIL!"
                    )
                if role == "assistant" and has_tool_result:
                    self._log_trace(
                        f"❌ VALIDATION ERROR: Assistant message {i} contains toolResult - THIS WILL FAIL!"
                    )
            ...
            if tool_config:
                ...
                self._log_trace("Tool config: None (tools disabled for this request)")
        ...
        try:
            ...
            self._log_trace("--- CALLING ORIGINAL METHOD ---")

            # Convert ChatOptions wrapper back to dict for the framework interface
            options_dict = chat_options.to_dict()

            chunk_count = 0
            async for chunk in self._original_get_streaming_response(
                messages=messages, options=options_dict, **kwargs
            ):
                chunk_count += 1
                ...
                if hasattr(chunk, "contents") and chunk.contents:
                    ...
                    for content in chunk.contents:
                        if isinstance(content, Content) and content.type == "text" and hasattr(
                            ...
                            cleaned_text = self._strip_markdown_code_blocks(
                                content.text
                            )
                            ...
                            if chunk_count <= 5 and cleaned_text != content.text:
                                self._log_trace(
                                    f"Stripped markdown code blocks from chunk {chunk_count}"
                                )
                                self._log_trace(
                                    f"Original length: {len(content.text)}, Cleaned length: {len(cleaned_text)}"
                                )
                ...
                if chunk_count <= 5:  # Log first few chunks
                    self._log_trace(
                        f"Response chunk {chunk_count}: {type(chunk).__name__}"
                    )
                    if hasattr(chunk, "contents") and chunk.contents:
                        for content in chunk.contents:
                            if hasattr(content, "text") and content.text:
                                ...
                                self._log_trace(
                                    f"Chunk text (after cleaning): {text_preview}"
                                )
                ...
                if hasattr(chunk, "contents") and chunk.contents:
                    for j, content in enumerate(chunk.contents):
                        ...
                        if hasattr(content, "output"):
                            content_info["output"] = str(content.output)[
                                :500
                            ]  # Limit size
            ...
            response_data = {"chunk_count": chunk_count, "chunks": detailed_chunks}
            ...
            if hasattr(e, "response"):
                error_details["boto3_response"] = {
                    ...
                    "HTTPStatusCode": e.response.get("ResponseMetadata", {}).get(
                        "HTTPStatusCode"
                    ),
                }
