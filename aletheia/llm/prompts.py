...
def get_user_prompt_template(template_name: str) -> PromptTemplate:
    ...
    This function first tries to load the template from files using PromptTemplateLoader.
    If not found, falls back to the hardcoded USER_PROMPT_TEMPLATES dictionary.
    ...
    Raises:
        ValueError: If template name is not recognized in either location
    """
...
def load_system_prompt(agent_name: str, mode: Optional[str] = None) -> str:
    ...
    Falls back to hardcoded SYSTEM_PROMPTS if template file not found.
    ...
    Raises:
        ValueError: If template is not found in files or hardcoded dict
    """
    # Build template name
    if mode:
    ...
    try:
        ...
        # Fall back to hardcoded prompts (with or without mode suffix)
        fallback_key = f"{agent_name}_{mode}" if mode else agent_name
        
        if fallback_key in SYSTEM_PROMPTS:
            return SYSTEM_PROMPTS[fallback_key]
        
        # If mode-specific not found, try base agent name
        if agent_name in SYSTEM_PROMPTS:
            return SYSTEM_PROMPTS[agent_name]
        
        raise ValueError(
            f"System prompt for '{agent_name}' (mode: {mode}) not found in templates or hardcoded prompts. "
            f"Available hardcoded: {list(SYSTEM_PROMPTS.keys())}"
        )
...
def load_user_prompt(template_name: str, **kwargs) -> str:
    ...
    Falls back to hardcoded USER_PROMPT_TEMPLATES if template file not found.
