...
class PCAPFilePlugin:
    ...
    def read_pcap_from_file(
        self, file_path: Annotated[str, "Path to the pcap file"]
    ) -> str:
        """Read a pcap file and return packet details as CSV (verbose mode)."""
        try:
            logger.debug(
                f"PCAPFilePlugin::read_pcap_from_file:: Reading PCAP file: {file_path}"
            )
            ...
            def packet_to_csv(packet, packet_num):
                ...
                if timestamp:
                    try:
                        ...
                        ts_str = datetime.datetime.fromtimestamp(ts_float).strftime(
                            "%Y-%m-%d %H:%M:%S.%f"
                        )
            ...
            logger.debug(
                f"PCAPFilePlugin::read_pcap_from_file:: returning {len(packets)} packets."
            )
            ...
            if self.session:
                saved = self.session.save_data(
                    SessionDataType.TCPDUMP, f"{filename}_tcpdump", "\n".join(csv_lines)
                )
                logger.debug(
                    f"PCAPFilePlugin::read_pcap_from_file:: Saved dump to {saved}"
                )
