...
class NetworkPlugin(BasePlugin):
    ...
    def is_ip_in_cidr(
        self, ip_address: Annotated[str, "The IP address to check"]
    ) -> bool:
        ...
        if not cidr_block:
            logger.error(
                "NetworkPlugin::is_ip_in_cidr:: No CIDR block found in session data"
            )
            return False
        try:
            ...
            logger.error(
                f"NetworkPlugin::is_ip_in_cidr:: Invalid IP address or CIDR block: {str(e)}"
            )
            return False

    def nslookup(self, domain: Annotated[str, "The domain name to look up"]) -> str:
        """Perform an nslookup for the given domain name."""
        command = ["nslookup", domain]
        return self._run_net_command(
            command, save_key="nslookup", log_prefix="NetworkPlugin::nslookup::"
        )
    ...
    def dig(
        ...
        dns_server: Annotated[str, "The DNS server to use"] = None,
    ) -> str:
        ...
        return self._run_net_command(
            command, save_key="dig", log_prefix="NetworkPlugin::dig::"
        )

    def ping(self, target: Annotated[str, "The host to ping"]):
        """Ping the specified host a given number of times."""
        command = ["ping", "-c", "3", target]
        return self._run_net_command(
            command, save_key="ping", log_prefix="NetworkPlugin::ping::"
        )
    ...
    def traceroute(self, target: str) -> str:
        ...
        return self._run_net_command(
            command, save_key="traceroute", log_prefix="NetworkPlugin::traceroute::"
        )
    ...
    def ifconfig(self) -> str:
        ...
        command = ["ifconfig"]
        return self._run_net_command(
            command, save_key="ifconfig", log_prefix="NetworkPlugin::ifconfig::"
        )
    ...
    def netstat(self) -> str:
        ...
        return self._run_net_command(
            command, save_key="netstat", log_prefix="NetworkPlugin::netstat::"
        )

    def whois(self, target: Annotated[str, "The host to lookup"]):
        """Perform a whois lookup for the given target."""
        command = ["whois", target]
        return self._run_net_command(
            command, save_key="whois", log_prefix="NetworkPlugin::whois::"
        )

    def openssl_sclient(
        self,
        target: Annotated[str, "The host to connect to"],
        port: Annotated[int, "The port to connect to"] = 443,
    ) -> str:
        """Perform an openssl s_client connection to the given target and port."""
        command = ["openssl", "s_client", "-connect", f"{target}:{port}"]
        return self._run_net_command(
            command,
            save_key="openssl_sclient",
